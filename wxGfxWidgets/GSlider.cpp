...
void GSlider::Render(wxGraphicsContext &gc)
{
	gc.PushState();
	gc.Translate((position.x + (c_back.GetWidth()/2.0)),(position.y + (c_back.GetHeight()/2.0)));
	gc.Rotate(rotation);
	gc.Scale(xScale, yScale);
	gc.Translate(-(position.x + (c_back.GetWidth()/2.0)),-(position.y + (c_back.GetHeight()/2.0)));
	gc.DrawBitmap( c_back, position.x, position.y, c_back.GetWidth(), c_back.GetHeight());
	for(unsigned int i=0; i<knobs.size(); ++i){
		gc.DrawBitmap( knobs[i].slider, position.x + knobs[i].knobPosX, position.y + knobs[i].knobPosY, knobs[i].slider.GetWidth(), knobs[i].slider.GetHeight());
	}
	gc.PopState();
}
