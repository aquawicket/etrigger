...
aubio_beattracking_t * new_aubio_beattracking(uint_t winlen,
	 ...
	 * 120bpm [43] */
	smpl_t rayparam = (smpl_t)(48./512. * winlen);
...
smpl_t fvec_getperiod(aubio_beattracking_t * bt){
	...
	for(i=0;i<numelem;i++){
		...
		//AUBIO_DBG("\n maxind is  %d\n",maxind);
		bt->inds->data[0][i] = (smpl_t)maxind;

	}

	for (i=0;i<numelem;i++){
		period += (smpl_t)(bt->inds->data[0][i]/(i+1.));}
