...
void aubio_window(smpl_t *w, uint_t size, aubio_window_type wintype) {
  ...
  switch(wintype) {
    ...
    case aubio_win_parzen:
      for (i=0;i<size;i++)
        w[i] = (smpl_t)(1.0 - ABS((2*i-size)/(size+1.0)));
      break;
    default:
      break;
  }
...
smpl_t aubio_unwrap2pi(smpl_t phase) {
  /* mod(phase+pi,-2pi)+pi */
  return (smpl_t)(phase + TWO_PI * (1. + FLOOR(-(phase+PI)/TWO_PI)));
}
...
void vec_alpha_normalise(fvec_t * mag, uint_t alpha)
{
  smpl_t alphan = 1.;
  uint_t length = mag->length, i=0, j;
  alphan = vec_alpha_norm(mag,(smpl_t)alpha);
  for (j=0;j<length;j++){
    mag->data[i][j] /= alphan;
  }
}
  ...
  if ((pos > span) && (pos < x->length-span)) {
    ...
    for (frac = 0.; frac < 2.; frac = frac + step) {
      ...
      if (res < resold) {
        ...
        exactpos += (smpl_t)((frac-step)*span - span/2.);
        break;
      }
    }
  }
...
smpl_t aubio_quadfrac(smpl_t s0, smpl_t s1, smpl_t s2, smpl_t pf) {
  smpl_t tmp = (smpl_t)(s0 + (pf/2.) * (pf * ( s0 - 2.*s1 + s2 ) - 3.*s0 + 4.*s1 - s2));
  return tmp;
}
...
smpl_t aubio_freqtomidi(smpl_t freq) {
        /* log(freq/A-2)/log(2) */
        smpl_t midi = (smpl_t)(freq/6.875);
        midi = (smpl_t)(LOG(midi)/0.69314718055995);
        midi *= 12;
        midi -= 3;  
        return midi;
}
