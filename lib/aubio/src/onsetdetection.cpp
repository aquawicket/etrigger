...
void aubio_onsetdetection_energy  (aubio_onsetdetection_t *o /*UNUSED*/, cvec_t * fftgrain, fvec_t * onset) {
	uint_t i,j;
  ...
  uint_t i;
	switch(type) {
		...
		case aubio_onset_phase:
			...
			o->theta1 = new_fvec(rsize,channels);
			o->theta2 = new_fvec(rsize,channels);
			o->histog = new_aubio_hist(0.0f, (smpl_t)PI, 10, channels);
