...
smpl_t aubio_pitchfcomb_detect (aubio_pitchfcomb_t * p, fvec_t * input)
{
  ...
  smpl_t freqPerBin = p->rate/(smpl_t)p->fftSize,
    phaseDifference = (smpl_t)(TWO_PI*(smpl_t)p->stepSize/(smpl_t)p->fftSize);
  ...
  for (k=0; k<=p->fftSize/2; k++) {
    smpl_t
      magnitude = (smpl_t)(20.*LOG10(2.*p->fftOut->norm[0][k]/(smpl_t)p->fftSize)),
    ...
    tmp = (smpl_t)(p->fftSize/(smpl_t)p->stepSize*tmp/(TWO_PI));
