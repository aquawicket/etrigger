...
void aubio_pitchmcomb_spectral_pp(aubio_pitchmcomb_t * p, fvec_t * newmag) {
  ...
  vec_alpha_normalise(mag,(uint_t)p->alpha); /* alpha normalisation  */
...
void aubio_pitchmcomb_combdet(aubio_pitchmcomb_t * p, fvec_t * newmag) {
  ...
  for (l=0;l<M;l++) {
    ...
    for (k=0;k<curlen;k++)
      candidate[l]->ecomb[k]=(smpl_t)((candidate[l]->ebin)*(k+1.));
    ...
    for (k=0;k<curlen;k++) {
      ...
      if ( 17. * xx < candidate[l]->ecomb[k] ) {
        ...
        candidate[l]->len += (smpl_t)(1./curlen);
      } else
        candidate[l]->ecomb[k]=0.;
    }
    /* punishment */
  ...
  for (i=0;i<X->channels;i++)
    for (j=1;j<X->length-1;j++)	{
      ...
      if (ispeak) {
        ...
        spectral_peaks[count-1].ebin = (smpl_t)(vec_quadint(X,j) - 1.);
      }
    }
  return count;
}
...
static sint_t aubio_pitchmcomb_sort_peak_comp(const void *x, const void *y) {
  return (sint_t)(((aubio_spectralpeak_t *)y)->mag - ((aubio_spectralpeak_t *)x)->mag);
}
...
aubio_pitchmcomb_t * new_aubio_pitchmcomb(uint_t bufsize, uint_t hopsize, uint_t channels, uint_t samplerate) {
  ...
  p->win_post         = 8;
  p->win_pre          = 7;
  p->tau              = (smpl_t)(samplerate/bufsize);
  p->alpha            = 9.;
  p->goodcandidate    = 0;
  p->phasefreq        = (smpl_t)(bufsize/hopsize/TWO_PI);
