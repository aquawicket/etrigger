...
struct bufferchain
{
	...
	ssize_t2 size;        /* Aggregated size of all buffies. */
	/* These positions are relative to buffer chain beginning. */
	ssize_t2 pos;         /* Position in whole chain. */
	ssize_t2 firstpos;    /* The point of return on non-forget() */
...
struct reader_data
{
	...
	ssize_t2 (*r_read_handle) (void *handle, void *buf, size_t count);
	...
	ssize_t2 (*read) (int fd, void *buf, size_t count);
	off_t   (*lseek)(int fd, off_t offset, int whence);
	/* Buffered readers want that abstracted, set internally. */
	ssize_t2 (*fullread)(mpg123_handle *, unsigned char *, ssize_t2);
