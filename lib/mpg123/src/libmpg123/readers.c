...
static ssize_t2 posix_read(int fd, void *buf, size_t count){ return read(fd, buf, count); }
...
static ssize_t2 plain_fullread(mpg123_handle *fr,unsigned char *buf, ssize_t2 count);
...
static ssize_t2 io_read(struct reader_data *rdat, void *buf, size_t count);

#ifndef NO_FEEDER
/* Bufferchain methods. */
static void bc_init(struct bufferchain *bc);
static void bc_reset(struct bufferchain *bc);
static int bc_append(struct bufferchain *bc, ssize_t2 size);
#if 0
static void bc_drop(struct bufferchain *bc);
#endif
static int bc_add(struct bufferchain *bc, const unsigned char *data, ssize_t2 size);
static ssize_t2 bc_give(struct bufferchain *bc, unsigned char *out, ssize_t2 size);
static ssize_t2 bc_skip(struct bufferchain *bc, ssize_t2 count);
static ssize_t2 bc_seekback(struct bufferchain *bc, ssize_t2 count);
...
static ssize_t2 plain_read(mpg123_handle *fr, void *buf, size_t count)
{
	ssize_t2 ret = io_read(&fr->rdat, buf, count);
...
static ssize_t2 timeout_read(mpg123_handle *fr, void *buf, size_t count)
{
	struct timeval tv;
	ssize_t2 ret = 0;
	fd_set fds;
...
static ssize_t2 icy_fullread(mpg123_handle *fr, unsigned char *buf, ssize_t2 count)
{
	ssize_t2 ret,cnt;
	cnt = 0;
...
static off_t stream_skip_bytes(mpg123_handle *fr,off_t len)
{
	...
	else if(len >= 0)
	{
		...
		while (len > 0)
		{
			ssize_t2 num = len < (off_t)sizeof(buf) ? (ssize_t2)len : (ssize_t2)sizeof(buf);
...
static ssize_t2 bc_need_more(struct bufferchain *bc)
{
...
static ssize_t2 bc_give(struct bufferchain *bc, unsigned char *out, ssize_t2 size)
{
	...
	{
		ssize_t2 loff = bc->pos - offset;
		ssize_t2 chunk = size - gotcount; /* amount of bytes to get from here... */
...
static ssize_t2 feed_read(mpg123_handle *fr, unsigned char *out, ssize_t2 count)
{
	ssize_t2 gotcount = bc_give(&fr->rdat.buffer, out, count);
...
static off_t feed_skip_bytes(mpg123_handle *fr,off_t len)
{
	...
	off_t res = bc_skip(&fr->rdat.buffer, (ssize_t2)len);
	if(res < 0) return res;
...
{
	if(bytes >=0)
	return bc_seekback(&fr->rdat.buffer, (ssize_t2)bytes) >= 0 ? 0 : READER_ERROR;
	else
...
off_t feed_set_pos(mpg123_handle *fr, off_t pos)
{
	...
	{ /* We have the position! */
		bc->pos = (ssize_t2)(pos - bc->fileoff);
...
static ssize_t2 buffered_fullread(mpg123_handle *fr, unsigned char *out, ssize_t2 count)
{
	struct bufferchain *bc = &fr->rdat.buffer;
	ssize_t2 gotcount;
	...
	{ /* Add more stuff to buffer. If hitting end of file, adjust count. */
		...
		ssize_t2 need = count - (bc->size-bc->pos);
		while(need>0)
		{
			int ret;
			ssize_t2 got = fr->rdat.fullread(fr, readbuf, BUFFBLOCK);
			if(got < 0)
			{
...
static ssize_t2 bad_fullread(mpg123_handle *mh, unsigned char *data, ssize_t2 count) bugger_off
